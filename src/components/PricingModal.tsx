    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl my-8 overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-8 max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between hover:border-emerald-500/50 transition relative group">
              ...
              <div className="mt-6 pt-4 border-t border-slate-900">
                {onSelectPlanDemo && (
                  <button
                    onClick={() => onSelectPlanDemo('LACI-DEMO-2026')}
                    className="w-full bg-slate-900 hover:bg-emerald-600 text-slate-200 hover:text-white py-2 rounded-xl text-xs font-medium transition cursor-pointer"
                  >
                    Uji Coba Kode Paket Ini
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-950 border-2 border-emerald-500/80 rounded-2xl p-5 flex flex-col justify-between shadow-xl shadow-emerald-950/30 relative">
              ...
              <div className="mt-6 pt-4 border-t border-slate-900">
                {onSelectPlanDemo && (
                  <button
                    onClick={() => onSelectPlanDemo('GURU-SUPER-9999')}
                    className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded-xl text-xs font-medium shadow-md transition cursor-pointer"
                  >
                    Uji Coba Kode Master (VIP)
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between hover:border-cyan-500/50 transition">
              ...
              <div className="mt-6 pt-4 border-t border-slate-900">
                {onSelectPlanDemo && (
                  <button
                    onClick={() => onSelectPlanDemo('TIBERSA-VIP-8888')}
                    className="w-full bg-slate-900 hover:bg-cyan-600 text-slate-200 hover:text-white py-2 rounded-xl text-xs font-medium transition cursor-pointer"
                  >
                    Uji Coba Kode Lembaga
                  </button>
                )}
              </div>
            </div>
          ...
          {/* Info Footer */}
          <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4 text-xs text-slate-300 flex items-center justify-between">
            <div>
              <span className="font-semibold text-white">Ingin Membeli Kode Akses Resmi?</span>
              <p className="text-slate-400 text-[11px]">
                Hubungi Admin Akademi Tibersa untuk pembelian lisensi resmi atau penawaran khusus sekolah.
              </p>
            </div>
            <button
              onClick={onClose}
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium px-4 py-2 rounded-xl transition cursor-pointer text-xs"
            >
              Mengerti & Kembali
            </button>
          </div>
