...
// SECURITY: read from an environment variable so the real key never lives
// in the source code / GitHub repo. Set ADMIN_KEY in Vercel → Settings →
// Environment Variables (Production, Preview, Development).
const ADMIN_KEY = process.env.ADMIN_KEY || 'CHANGE-THIS-BEFORE-DEPLOY';
...
// (see api/index.ts). On Vercel this file is never executed directly,
// so app.listen() below never runs there.
export default app;
...
// Only boot the persistent server when running locally (npm run dev / npm start).
// On Vercel, the VERCEL env var is always set, so this block is skipped.
